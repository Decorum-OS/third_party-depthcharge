...
class Gbb(Area):
  ...
  def write(self):
    ...
    gbb_utility.create([0x100, 0x1000, self.placed_size - 0x2180, 0x1000], gbbp)
    ...
    for f in bmpfv, rootkey, recoverykey:
      f.close()
    gbb_utility.set(self._hwid, self._flags, bmpfvp, rootkeyp, recoverykeyp)
